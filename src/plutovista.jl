...
function initialize!(p::GridVisualizer, ::Type{PlutoVistaType})
    ...
    for I in CartesianIndices(layout)
        ...
        PlutoVista.backend!(ctx[:figure]; backend = p.context[:backend],
...
function gridplot!(ctx, TP::Type{PlutoVistaType}, ::Type{Val{1}}, grid)
    ...
    for icell = 1:num_cells(grid)
        ...
        PlutoVista.plot!(ctx[:figure], [x1, x2], [0, 0]; clear = false, linewidth = 3.0,
                         color = rgbtuple(cmap[cellregions[icell]]), label = label)
        PlutoVista.plot!(ctx[:figure], [x1, x1], [-h, h]; clear = false,
                         linewidth = ctx[:linewidth], color = :black)
        PlutoVista.plot!(ctx[:figure], [x2, x2], [-h, h]; clear = false,
                         linewidth = ctx[:linewidth], color = :black)
    end
    ...
    for ibface = 1:num_bfaces(grid)
        ...
        if ireg > 0
            ...
            PlutoVista.plot!(ctx[:figure], [x1, x1], [-2 * h, 2 * h]; clear = false,
                             linewidth = 3.0, color = rgbtuple(cmap[ireg]), label = label,
